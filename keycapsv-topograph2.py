...
CONTOUR_LEVELS = 10       # The number of contour lines to draw.
...
def generate_keycap_grid():
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            # FIX: Correct typo from top__base to top_base
            full_top = top_base
            if pattern:
                ...
                # FIX: Process the mesh to merge vertices. This helps ensure the deformation
                # is applied to a single, unified surface.
                full_top.process(validate=True)


            # 4. Apply deformation to the unified top surface for the fingertip dish
            vertices = full_top.vertices
            
            # Select vertices based on face normals to ensure only the top "skin" is deformed.
